...
//            $("#videoPreview").attr("src", embedUrl);
            $("#videoPreview").append('<iframe width="100%" height="350px" class="embed-responsive-item" src="'+embedUrl+'" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
        }
        else{
...
//        alert(file_length);
        for(var i=0;i<file_length;i++){
            $('#previewImages').append("<div class='col-md-3'><img class='preview-image' src='"+URL.createObjectURL(event.target.files[i])+"'></div><br>");
        }
    });
//        select images
});
